...
def checkAnyHighCorr(corrsAll,scoresList): ### for any extremely high correlations set scores the same
    highCorr=0.99
    for key,corr in corrsAll.items():
        if corr>highCorr:
            scoresList[key[0]]=scoresList[key[1]]
            print('here')
    return scoresList
    
def returnUpdatedScores(scoresList,corrsAll,args):
    # print(corrsAll,len(scoresList))
    scoresList=checkAnyHighCorr(corrsAll,scoresList)
    
    tolerance,iterationLimit=args['tolerance'],args['iterLimit']
    keepRunning=True
